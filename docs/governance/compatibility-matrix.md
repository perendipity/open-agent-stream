...
| `webhook` | `sink/v1` | `side_effecting` | `options.url` | reference implementation | POSTs event batches to an external HTTP receiver. |
...
| Sink authoring | supported at the contract level | `spec/sink/v1`, `pkg/sinkapi`, `pkg/schema`, replay-class documentation model | dynamic registration in the stock `oas` CLI |
